...
function serviceValidate (req, reply) {
  req.log(['debug', 'protocol3'], `got /p3/serviceValidate ${req.method.toUpperCase()}`)
  const service = decodeURIComponent(getParameter(req, 'service'))
  const stId = getParameter(req, 'ticket')

  if (!stId || !service) {
    req.log(['debug', 'protocol3'], 'missing required parameters: st and/or service')
  ...
  req.log(['debug', 'protocol3'], `serviceValidate parameters: (service: ${service}, stId: ${stId}, pgtUrl: ${pgtUrl}, renew: ${renew})`)
  ...
  function * doValidate () {
    ...
    let st
    try {
      req.log(['debug', 'protocol3'], `validating service ticket: ${stId}`)
      st = yield cas.getServiceTicket(stId)
      if (st.expired) throw new Error('service ticket expired')
      req.log(['debug', 'protocol3'], 'valid service ticket')
    } catch (e) {
      req.log(['error', 'protocol3'], 'invalid service ticket')
      return reply(xml.invalidST.stream({
    ...
    let tgt
    try {
      req.log(['debug', 'protocol3'], `getting tgt for st: ${stId}`)
      tgt = yield cas.getTicketGrantingTicketByServiceTicket(stId)
      req.log(['debug', 'protocol3'], `got tgt: ${tgt.tid}`)
    } catch (e) {
      req.log(['error', 'protocol3'], `unable to get tgt for st: ${stId}`)
      return reply(xml.invalidST.stream({
        code: errorCodes.INVALID_TICKET,
    ...
    try {
      req.log(['debug', 'protocol3'], `invalidating st: ${stId}`)
      st = yield cas.invalidateServiceTicket(stId)
      req.log(['debug', 'protocol3'], `service ticket invalidated: ${st.tid}`)
    } catch (e) {
      req.log(['error', 'protocol3'], 'could not invalidate service ticket')
      return reply(xml.invalidST.stream({
    ...
    for (const plugin of Object.keys((hooks) ? hooks.userAttributes : {})) {
      try {
        ...
        req.log(['debug', 'protocol3'], `retrieved attributes: ${JSON.stringify(data)}`)
        attributes = Object.assign(attributes, data)
      } catch (e) {
        req.log(['error', 'protocol3'], `could not retrieve user attributes via plugin '${plugin}': ${e.message}`)
        req.log(['debug', 'protocol3'], e.stack)
      }
    }
    ...
    if (service) {
      try {
        ...
        req.log(['debug', 'protocol3'], `tracking service for logout: ${st.serviceId}`)
    ...
    req.log(['debug', 'protocol3'], `returning ${tgt.userId}: ${JSON.stringify(attributes)}`)
